...
*/}}
{{- define "daytona.serviceEnv" -}}
{{- $service := .service -}}
{{- $values := .values -}}
{{- $context := .context -}}
{{- $serviceConfig := index $values.services $service -}}
{{- range $key, $value := $serviceConfig.env }}
{{- if and (eq $service "api") (eq $key "REDIS_HOST") (eq $value "") }}
{{- else if and (eq $service "api") (eq $key "DB_PASSWORD") }}
{{- else }}
- name: {{ $key }}
  value: {{ $value | quote }}
{{- end }}
{{- end }}
{{- if eq $service "api" }}
{{- if eq $serviceConfig.env.REDIS_HOST "" }}
- name: REDIS_HOST
  value: {{ printf "%s-redis-master" $context.Release.Name | quote }}
{{- end }}
{{- if $values.redis.auth.enabled }}
- name: REDIS_PASSWORD
  valueFrom:
    secretKeyRef:
      name: {{ printf "%s-redis" $context.Release.Name }}
      key: redis-password
{{- end }}
{{- if not (hasKey $serviceConfig.env "DB_PASSWORD") }}
- name: DB_PASSWORD
  valueFrom:
    secretKeyRef:
      name: {{ include "daytona.secretName" $context }}
